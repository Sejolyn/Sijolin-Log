...
在 CS144 中，TCP 默认使用**分组交换**。

报文（Message）在应用层生成后，在传输层（TCP）中会被切分成小的 TCP 段（增加 TCP 头），继续在网络层中加上 IP 头形成分组。
在分组交换中，发送方会把数据切成一段一段的分组丢到网络上，接收方再想办法拼回原来的字节流。问题是网络环境并不老实，分组可能乱序、可能丢、甚至可能重复。所以我们要写一个小组件 —— **重组器 (Reassembler)**，来处理这些情况。
